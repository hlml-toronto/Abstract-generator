...
class CustomBatch():
    ...
    def __init__(self, data, dim=0, pad_value=0, max_len_model=None):
        ...
        self.src = (torch.stack([x[:-1] for x in batch], dim=1 - dim)).long()
        self.tgt = (torch.stack([x[1:] for x in batch], dim=1 - dim)).long()
        '''src_pad_mask: :math:(N, S) where N is the batch size, S is
    ...
    def pad_tensor(self, vec):
        ...
        pad_size[0] = self.maxLen - vec.size(0)
        return torch.cat([vec, self.padValue * torch.ones(*pad_size)],
                         dim=0)
